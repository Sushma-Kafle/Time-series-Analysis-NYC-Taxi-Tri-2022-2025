---
title: "ETS_Updated_Taxi_Trip_Analysis"
format: html
editor: visual
---
...
```{r}
#| echo: false
data_folder <- "C:/Users/patel/OneDrive/Documents/R projects/ADS506_TaxiData"
...
ets_fit <- train_taxi |>
  model(ets = ETS(trips))
fc_ets <- ets_fit |> forecast(h = "180 days", level = c(80, 95))
```

```{r}
recent_dates <- daily_ts %>% 
  filter(date >= as.Date("2024-10-01"))

autoplot(fc_ets, recent_dates) +labs(title = "ETS Forecast of Daily Yellow Taxi Trips",x = "Date",y = "Number of Trips" )

ets_acc_tbl <- accuracy(fc_ets, test_taxi,measures = list(ME = ME, RMSE = RMSE, MAE = MAE, MAPE = MAPE, MPE = MPE, ACF1 = ACF1))
  ets_acc_tbl|>
  gt() |>
  tab_header(title = "Model Accuracy on Test Set") |>
  fmt_number(columns = where(is.numeric),decimals = 2)
